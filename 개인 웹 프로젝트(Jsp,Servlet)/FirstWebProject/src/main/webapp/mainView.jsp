...
<!-- 여기서부터 복사 -->
          ...
          <ul class="navbar-nav mr-auto">
            ...
            <%= name %>님 안녕하세요.
				<form action="logoutOk.do" method="post">
					...
					<button class="btn btn-outline-secondary" onclick="nvlogout()"></button>
				</form>
            </li>
          </ul>
        </div>
    ...
    <!-- 여기 까지 복사 -->
		...
		<h1><%= name %>님의 메인 페이지입니다.</h1>
		<img alt="mainImage" src="mainImage.jpg" width="70%"> 
		<c:if test = "${'admin' eq (sessionScope.id)}">
            <div><a href="adminView.jsp">관리자만 보이는 관리자메뉴</a></div>
 		</c:if>
