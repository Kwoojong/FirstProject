...
function submit_ajax() {
   ...
   $.ajax({
       url : '/FirstWebProject/iListView.do',
       type : 'POST',
       data : queryString,
       dataType: 'text',
       success : function(json) {  
    	   window.location.replace('iListView.do?search='+search+'&searchSomething='+searchSomething);
       }
   });
}
</script>
        ...
        <tr class="table-active">
       	...
       	<tbody>	
       		<tr>
                ...
                <td>${dto.bName}</td>
                <td>
      	            <a href="iContentView.do?bId=${dto.bId}">${dto.bTitle}</a>
		...
		<tr>
            ...
            <c:choose>
                <c:when test="${(page.curPage-1)<1}">
                    <span class="page-link" href="iListView.do?page=1">처음</span>
                </c:when>
                <c:otherwise>
                    <a class="page-link" href="iListView.do?page=1">처음</a>
                </c:otherwise>
            </c:choose>
            </li>
            ...
            <c:choose>
                <c:when test="${(page.curPage-1) < 1}">
                    <span class="page-link" href="iListView.do?page=${page.curPage-1}">이전</span>
                </c:when>
                <c:otherwise>
                    <a class="page-link" href="iListView.do?page=${page.curPage-1}">이전</a>
                </c:otherwise>
            </c:choose>
			</li>
            <!--개별 페이지-->
            <c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
                <c:choose>
                    ...
                    <c:otherwise>
                        <li class="page-item"><a class="page-link" href="iListView.do?page=${fEach}">${fEach}</a></li>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:choose>
                <c:when test="${(page.curPage +1) > page.totalPage}">
                    <span class="page-link" href="iListView.do?page=${page.curPage+1}">다음</span>
                </c:when>
                <c:otherwise>
                    <a class="page-link" href="iListView.do?page=${page.curPage+1}">다음</a>
                </c:otherwise>
            </c:choose>
			</li>
            <!--끝-->
            ...
            <c:choose>
                <c:when test="${page.curPage == page.totalPage}">
                    <span class="page-link" href="iListView.do?page=${page.totalPage}">끝</span>
                </c:when>
                <c:otherwise>
                    <a class="page-link" href="iListView.do?page=${page.totalPage}">끝</a>
                </c:otherwise>
            </c:choose>
            </li>
            </ul>
            ...
            <td><a href="iwriteView.do" class="btn btn-primary" tabindex="-1" role="button" aria-disabled="true">글작성</a></td>
		</tr>
		</tbody>
    </table>
    </div>
